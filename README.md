...
- `peek` command to check the key's type then automatic call `get`/`lrange`/`sscan`,etc depends on types. You don't need to call `type` command then type another command to get value.
