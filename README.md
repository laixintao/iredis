...
Try iredis on Google Clour Run(Note GCR will try to deploy this image to cloud run, but we actually don't need to deploy, it's just a cli tool, so you just run `docker run -it you-built-image` after deply fails.)
