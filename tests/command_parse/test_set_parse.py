def test_sadd(judge_command):
    judge_command(
        "SADD foo m1 m2 m3",
        {"command": "SADD", "key": "foo", "members": "m1 m2 m3"},
    )
    judge_command(
        "SADD foo m1", {"command": "SADD", "key": "foo", "members": "m1"}
    )
...
def test_is_member(judge_command):
    ...
    judge_command(
        "SISMEMBER foo m1",
        {"command": "SISMEMBER", "key": "foo", "member": "m1"},
    )
...
def test_spop(judge_command):
    ...
    judge_command(
        "SPOP set 3", {"command": "SPOP", "key": "set", "count": "3"}
    )
